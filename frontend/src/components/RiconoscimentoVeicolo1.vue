...
onMounted(() => {
  ...
  _paq.push(['FormAnalytics::scanForForms', document.getElementById('formRv1' + (dataStore.getIsVariantOriginal ? 'A' : 'B'))]);
  _paq.push(['FormAnalytics::trackForm', document.getElementById('formRv1' + (dataStore.getIsVariantOriginal ? 'A' : 'B'))]);
})
...
const okFunction = () => {
  ...
  var _paq = (window._paq = window._paq || []);
  _paq.push(['FormAnalytics::trackFormSubmit', document.getElementById('formRv1' + (dataStore.getIsVariantOriginal ? 'A' : 'B'))]);
  fetch("/public/api/targa", {
    method: "POST",
        ...
        response.json().then((json) => {
          ...
          if (res.success == "0") {
            ...
            if (res.response.copertura == "1") {
              ...
              router.push("/endKo");
              _paq.push(['trackEvent', 'WayOut', 'RV1', 'TargaNonCoperta'])
            }
          } else {
            router.push("/rv1telaio");
          }
        });
      }
    })
...
<template>
  <form autocomplete="on" :id="'formRv1' + (dataStore.getIsVariantOriginal ? 'A' : 'B')">
    <div class="w-full text-center items-stretch flex flex-col py-8 pt-20 min-h-[95vh]">
      ...
      <div class="w-full flex-1 px-8 pt-4">
        ...
        <div class="pt-8">
          <Input v-model="targa" :placeholder="$t('dati.targa_placeholder')" :text="$t('dati.targa')" :required="true"
            validation-regex="^([A-Za-z]{2}){1}(\d{3}){1}([0-9A-Za-z]{2}){1}$" @is-valid="(valid) => (targaOk = valid)"
            name="plate" />
        </div>
      </div>
      ...
      <div class="w-full px-8">
        <Button :text="$t('avanti')" :disabled="!numCellOk || !targaOk" :onclick="($event) => okFunction()" />
      </div>
    </div>
  </form>
</template>
